...
INSERT INTO `devops_custom_pipeline_temp_stage` (`id`, `template_id`, `name`, `code`, `stage_order`, `stage`, `create_by`, `create_time`, `update_by`, `update_time`, `del_flag`, `version`) VALUES ('f6b5010845ee41d4863931c9e71237f1', '2_maven_single_temp_stage', '构建镜像', 'build-image', 3, '{\r\n	\"image\": \"registry.cn-hangzhou.aliyuncs.com/goodrain/docker:20.10.9\",\r\n	\"script\": [\"cp ${package_path}/target/${jar_name}.jar app.jar\", \"docker login -u ${REPOSITORY_USERNAME} -p ${REPOSITORY_PASSWORD} ${REPOSITORY_URL}\", \"docker build -t  ${REPOSITORY_URL}/${ORG}/${MODULE}:${DEVOPS_VERSION} .\", \"docker push ${REPOSITORY_URL}/${ORG}/${MODULE}:${DEVOPS_VERSION}\"],\r\n	\"stage\": \"build-image\"\r\n}', 'admin', '2022-08-24 08:39:05', 'admin', '2022-08-24 08:39:11', NULL, NULL);
